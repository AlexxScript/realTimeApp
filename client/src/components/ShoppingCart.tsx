...
interface propCart {
    modalShoppingCart: boolean;
    handleButtonModalCart: () => void;
}

export const ShoppingCart = (props:propCart) => {
    ...
    const makeOrder = () => {
        socket.emit("makeOrderClient", { cart: JSON.stringify(cart), totalAcum:total, idSchool: contextAu.user.idSchool, email: contextAu.user.email });
    ...
    return (
        ...
        <div className={`bg-slate-800 bg-opacity-50 justify-center items-center top-0 right-0 bottom-0 left-0 flex-1 fixed ${props.modalShoppingCart ? 'flex' : 'hidden'}`}>
                ...
                <div className="relative ">
                    <div className="flex flex-row cursor-pointer truncate p-2 px-4  rounded">
                        <div className="flex flex-row-reverse ml-2 w-full ">
                            <button onClick={props.handleButtonModalCart} slot="icon" className="relative">
